...
console.log();
console.log(
  `Você é um fisioculturista e esta se preparando para o Mr.Olympia. Faltando somente 5 dias para a competição, você tem que decidir qual será o melhor exercício para cada dia.\n`
);

const pergunta1 = prompt(
  `É o primeiro dia de treino, você quer treinar peito? (sim/nao)  `
).toLowerCase();
const pergunta2 = prompt(
  `É o segundo dia de treino, você quer treinar perna? (sim/nao)  `
).toLowerCase();
const pergunta3 = prompt(
  `É o terceiro dia de treino, você quer treinar costas? (sim/nao)  `
).toLowerCase();
const pergunta4 = prompt(
  `É o quarto dia de treino, você quer treinar tríceps? (sim/nao)  `
).toLowerCase();
const pergunta5 = prompt(
  `É o quinto dia de treino, você quer treinar bíceps? (sim/nao)  `
).toLowerCase();
...
for (i = 0; i < perguntas.length; i++) {
  while (perguntas[i] != "sim" && perguntas[i] != "nao") {
    perguntas[i] = prompt(
      `Resposta inválida na pergunta ${i + 1}. Digite somente sim ou nao: `
    );
  }
  if (perguntas[i] == "sim") {
    ganhos++;
